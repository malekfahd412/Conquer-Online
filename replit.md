...
## Status
Running on Replit (re-imported and re-set-up again on July 14, 2026 — `node_modules` was missing after import and secrets (`DISCORD_BOT_TOKEN`, `GEMINI_API_KEY`) were lost again; `CHANNEL_SERVER_STATUS` is baked into `.replit` as a shared env var so it persisted. Dependencies reinstalled via `pnpm install`, secrets re-requested and confirmed working, bot logged in, registered 33 guild commands in both guilds, found the status channel, and Ticket System Pro (13 engines) came online cleanly). AI provider = gemini. Shared env var `CHANNEL_SERVER_STATUS` (baked into `.replit`). `DATA_SOURCE` is unset, so live server status shows "Waiting for Server Connection" (mock/mssql/api not wired). Voice STT/TTS (`OPENAI_API_KEY` or other providers) not configured — voice AI features are inactive but text/slash AI commands work. Confirmed working: bot logs in, registers 9 guild commands, finds the status channel, and the Ticket System Pro (11 engines) comes online. The `artifacts/api-server` and `artifacts/mockup-sandbox` workflows also run (auto-added by the platform); they are auxiliary to the Discord bot and not required for it to function.
