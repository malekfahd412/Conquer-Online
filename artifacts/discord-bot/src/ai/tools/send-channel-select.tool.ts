import {
  ActionRowBuilder, ChannelSelectMenuBuilder, EmbedBuilder, ChannelType,
} from 'discord.js';
import type { Guild, TextChannel } from 'discord.js';
import type { ITool, ToolDefinition, ToolExecuteResult } from './tool.interface';
import { parseColor, resolveThemeColor } from './embed-themes';

export class SendChannelSelectTool implements ITool {
  readonly definition: ToolDefinition = {
    name: 'send_channel_select',
    description: 'Sends a message with a channel select menu — lets users pick one or more channels from a Discord-native dropdown.',
    parameters: {
      type: 'object',
      properties: {
        channel: { type: 'string', description: 'Channel name or ID to send the select to' },
        placeholder: { type: 'string', description: 'Placeholder text' },
        min_values: { type: 'string', description: 'Minimum channels to select (default: 1)' },
        max_values: { type: 'string', description: 'Maximum channels to select (default: 1)' },
    ...
    dangerous: false,
    examples: ['Send a channel select to #admin so admins can pick a log channel'],
  };
  ...
  async execute(params: Record<string, unknown>, guild: Guild): Promise<ToolExecuteResult> {
    ...
    const select = new ChannelSelectMenuBuilder()
      .setCustomId(`ch_select_${Date.now()}`)
      .setPlaceholder(String(params['placeholder'] ?? 'Select a channel'))
    ...
    return {
      success: true,
      message: `✅ Channel select menu sent to **#${ch.name}** (message ID: \`${sent.id}\`)\n⚠️ Requires an interaction handler to act on the selected channel.`,
