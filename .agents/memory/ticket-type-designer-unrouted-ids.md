...
Fixed by adding a minimal `tp:tt:` route: a read-only "Ticket Type Settings" hub (`buildTTMain` in tp-renderer.ts) showing the resolved per-type config plus a "Clear Overrides" action (`setEntryOverrides(panel, ref, {})`). Per-field section editors (roles/categories/claim/etc. sub-buttons already generated by `TP.TT.*`) are intentionally still unbuilt — clicking them now returns a clear ephemeral notice instead of failing, scoped this way deliberately to avoid rebuilding the whole designer in a bug-fix pass.
